...
var _ground_cooldown_until_s: float = -INF
var _queued_grounding_velocity: Vector3 = Vector3.ZERO
var _queued_grounding_slide_until_s: float = -INF
...
func setup(
	...
	if config == null:
		push_error(
			"SlideAction requires SlideConfig."
		)
		set_process(false)
		return
...
func can_start(
	...
	if context.time_s < _ground_cooldown_until_s:
		return false
...
func start(
	...
	var has_queued_grounding_slide: bool = (
		context.time_s
		<= _queued_grounding_slide_until_s
	)

	if has_queued_grounding_slide:
		current_horizontal = _queued_grounding_velocity

		_queued_grounding_velocity = Vector3.ZERO
		_queued_grounding_slide_until_s = -INF
...
func finish(
	...
	if context.is_grounded:
		_ground_cooldown_until_s = maxf(
			_ground_cooldown_until_s,
			context.time_s
			+ config.ground_cooldown_s
		)
...
func _apply_steering(
	...
	var current_speed_mps: float = (
		_slide_velocity.length()
	)

	var current_direction: Vector3 = (
		_slide_velocity.normalized()
	)

	var steering_weight: float = minf(
		config.steering_lerp_per_second
		* context.delta,
		1.0
	)
...
func _apply_slope_acceleration(
	context: MovementContext
) -> void:
...
func _apply_deceleration(
	context: MovementContext
) -> void:
	_slide_velocity = _slide_velocity.move_toward(
		Vector3.ZERO,
		config.flat_deceleration_mps2
		* context.delta
	)


func queue_grounding_slide(
	entry_horizontal_velocity: Vector3,
	current_time_s: float,
	window_s: float
) -> void:
	_queued_grounding_velocity = entry_horizontal_velocity
